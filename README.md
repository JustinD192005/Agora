...
- 4/5 researchers completed successfully; 1 hit iteration cap
...
- **No deterministic replay yet** — re-running the same question hits the LLM again. Response caching is on the Week 5 roadmap.
- **No dashboard yet** — run state is queryable via database but there's no UI. Week 4 work.
...
- **Week 4:** Next.js + shadcn/ui dashboard with WebSocket-streamed run state and a failure-trace visualization.
- **Week 5:** Eval harness (~30 benchmark questions, LLM-judge scoring), deterministic replay via response caching, source-agreement visualization.
- **Week 6:** Fly.io + Neon + Upstash deployment, demo video, blog post.
